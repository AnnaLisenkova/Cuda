...
int main(int argc, char* argv[]){
	...
	//int numbOfBlock;
	//int numbOfThread;
	for (int numbOfBlock = 1; numbOfBlock <= columns; numbOfBlock++ )
	{
		for(int numbOfThread = 0; numbOfThread <= columns; numbOfThread++){
...
ofstream fout("test.txt");
  for (it = Results.begin(); it != Results.end(); ++it)//запись в файл
  {
